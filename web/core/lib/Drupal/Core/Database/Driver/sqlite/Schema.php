  ...
  public function addField($table, $field, $specification, $keys_new = []) {
    ...
    if (empty($keys_new) && (empty($specification['not null']) || isset($specification['default']))) {
      ...
      if (isset($specification['initial_from_field'])) {
        if (is_array($specification['initial_from_field'])) {
          $expression = 'COALESCE(' . $specification['initial_from_field']['field_name'] . ', :default_initial_value)';
          $arguments = [':default_initial_value' => $specification['initial_from_field']['default_value']];
        }
        else {
